...
- **Pocket radio:** tap the speaker grille three times, with at most 1.2 seconds between taps, while the console is powered on and facing forward, with no dialog or Byte Snake game open. Click/touch and native Enter/Space activation work; held keys do not count repeatedly. This fourth discovery opens a music card with the supplied Moonlight recording and a personal invitation to explore below. The background fades into a pixel-textured night, with a soft white wave and floating particles. Scrolling toward the light gradually fades the portfolio out and reveals a white, black and gold piano salon, an animated pianist and the dedication **Te dedico mi melodía favorita**. The supplied Chopin recording replaces Moonlight on entry; one audio element prevents overlapping tracks.
...
The agreed implementation queue and the two upcoming secrets are in [docs/EASTER_EGGS.md](docs/EASTER_EGGS.md).
