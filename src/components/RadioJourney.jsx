...
export default function RadioJourney({ t, sound, onToggleSound, audioStatus, onTogglePlayback, onClose, onSceneChange, onPortfolioHidden, portfolioRef, startScroll }) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const target = scene === 'piano' ? roomRef.current?.querySelector('h2') : ticketTitleRef.current;
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      journeyRef.current.style.setProperty('--room-reveal', state.reveal);
  ...
  }, [onPortfolioHidden, onSceneChange, portfolioRef, startScroll]);
  ...
  return <div className="radio-journey" ref={journeyRef} data-scene={scene} data-paused={tabHidden}>
    ...
    <aside className="radio-ticket" aria-labelledby="radio-ticket-title" inert={scene === 'piano' ? '' : undefined} aria-hidden={scene === 'piano' || undefined}>
