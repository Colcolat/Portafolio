...
export default function SecretsDialog({ view, foundIds, onView, onClose, t }) {
  ...
  const discovery = secretCatalog.find(secret => secret.view === view && foundIds.includes(secret.id));
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      queueMicrotask(() => {
        ...
        // A rear control may have turned edge-on while the dialog settled the tilt.
        if (document.activeElement !== previousFocus || previousFocus === document.body) {
          document.getElementById('secrets-found')?.focus();
        }
      });
    };
  }, []);

  useEffect(() => { titleRef.current?.focus(); }, [discovery]);
  ...
  return <dialog className="reader secrets-dialog" ref={dialogRef} aria-labelledby="secrets-title"
    ...
    <div className="secrets-dialog-content">
      ...
      {discovery ? discovery.id === 'backend' ? <BackendRoom t={t} /> : <DeveloperRoom t={t} /> : <>
        ...
        <ul className="secret-collection">{secretCatalog.map((secret, i) => {
          ...
          return <li key={secret.id} className={found ? 'secret-card discovered' : 'secret-card'}>
            ...
            {found ? <button type="button" className="secret-card-link" onClick={() => onView(secret.view)}><span><strong>{t(secret.title)}</strong><small>{t(secret.description)}</small></span><span aria-hidden="true">↗</span></button>
              : <div className="secret-card-locked"><strong>???</strong><span>{t('Not discovered yet')}</span><small>{t(secret.hint)}</small></div>}
